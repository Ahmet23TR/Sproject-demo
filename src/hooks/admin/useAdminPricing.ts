...
export const useAdminPricing = () => {
    ...
    const handleCreate = async (name: string, isDefault?: boolean) => {
        if (!token) return;
        setCreating(true);
        try {
            await createPriceList({ name, isDefault }, token);
            await refetch();
        } catch (err) {
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(true);
        try {
            const result = await deletePriceList(id, token);
            ...
            if (result?.data?.reassignedUsersCount && result.data.reassignedUsersCount > 0) {
                ...
                const message = `Price list deleted successfully. ${userCount} user(s) have been automatically reassigned to the default price list.`;
    ...
    return { lists, defaultList, loading, error, success, creating, deleting, refetch, onCreate: handleCreate, onDelete: handleDelete, clearSuccess };
};

export const useAdminPriceListDetail = (id: string) => {
    const { token } = useAuth();
    ...
    const refetch = useCallback(async () => {
        ...
        setError("");
        try {
            const data = await fetchPriceListDetail(id, token);
            setDetail(data);
        } catch (err) {
    ...
    const handleSave = async (items: Array<{ optionItemId: string; price?: number; multiplier?: number }>) => {
        ...
        setSaving(true);
        try {
            const updated = await updatePriceListItems(id, items, token);
            setDetail(updated);
        } catch (err) {
    ...
    const handleSetDefault = async () => {
        ...
        setSaving(true);
        try {
            const updated = await setPriceListDefault(id, token);
            setDetail(updated);
        } catch (err) {
    ...
    return { detail, loading, error, saving, refetch, onSave: handleSave, onSetDefault: handleSetDefault };
};
